package com.michaelfotiadis.mobiledota2.data.persistence.error;
    ...
    public static UiDataLoadError createError(final Context context,
        ...
        if (error == null) {
            Answers.getInstance().logCustom(new CustomEvent("Error")
                    .putCustomAttribute("Content", "INVALID")
                    .putCustomAttribute("HTTP Code", -1));
        } else {
            Answers.getInstance().logCustom(new CustomEvent("Error")
                    .putCustomAttribute("Content", error.getKind().toString())
                    .putCustomAttribute("HTTP Code", error.getHttpStatus()));
        }
        ...
        } else if (error.getHttpStatus() == 503) {
            return createServerError(context);
        } else {
            AppLog.w("Error " + error.getKind());
